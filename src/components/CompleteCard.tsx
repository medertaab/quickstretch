...
import CompleteCardStyled from "./styles/CompleteCardStyled";

export default function CompleteCard(props: any) {
  const { title, setIsComplete, progress, mode } = props;
  const [shareButtonText, setShareButtonText] = useState("Share")

  const isCompletedSet = Object.values(progress).every(value => value === true)
  const today = new Date().toLocaleDateString()

  // Handle streak local storage
  if (isCompletedSet) {
  ...
  return (
    <CompleteCardStyled className="fade-in">
      <div className="container slide-up">
        ...
        {!isCompletedSet && <p className="notice">Complete the entire set to increase your streak</p>}
